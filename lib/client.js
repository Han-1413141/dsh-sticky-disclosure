/**
 * dsh-sticky-disclosure — browser half (v0.2).
 *
 ...
 *
 * v0.2 adds an always-visible presence:
 *   - a floating "collapse all" pill at the bottom-right of the conversation
 *     scrollport, with a live count of expanded sections — it appears the
 *     moment the plugin loads, so the plugin being active is immediately
 *     visible without scrolling anything off screen;
 *   - the hotkey (and the pill) now collapse EVERY expanded section in the
 *     conversation, visible or not, so the effect is observable on the spot;
 *   - a console.info line on apply, and a small debug handle
 *     `window.dshStickyDisclosure` for support.
 *
  ...
  factory: (require) => {
    ...
    function installStyles() {
      ...
      const css = [
        ...
        "@media (prefers-reduced-motion:reduce){.dshSd_chip,.dshSd_control{animation:none}}",
      ].join("");
    ...
    function apply(ctx) {
      ...
      function createControl() {
        ...
        btn.title = "收起全部展开区块 · " + HOTKEY_LABEL;
      ...
      /** Keep the floating pill present, counted, and pinned to the scrollport's bottom-right corner. */
      function syncControl() {
        ...
        control.style.left = Math.max(0, Math.round(spRect.right - CONTROL_INSET - w)) + "px";
        control.style.top = Math.max(0, Math.round(spRect.bottom - CONTROL_INSET - h)) + "px";
      }
      ...
      ctx.effect(() => {
        ...
        try {
          console.info("[dsh-sticky-disclosure] applied v" + PLUGIN_VERSION + " — " + HOTKEY_LABEL + " collapses all expanded sections");
        } catch (_) {
